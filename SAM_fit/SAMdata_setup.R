...
comm_wt$'2' <-sw[,1] #using sw for age2 for now, Jon extrapolation migth be usefull here
...
#both <- as.data.frame(dcast(indices[indices$survey == "BOTH",],Year~Age,value.var = 'index')) ##not working, probably more than 1 survey
...
#write.csv(geac,'C:/Users/jchampag/Dropbox/IFREMER/3Ps_cod/Cours/Cours_Noel_Cadigan/F6005_winter_2019/Project1/GEAC.csv')
...
dim(surveys$`GEAC													`);dim(surveys$`IO													`);dim(surveys$`ERHAPS													`);dim(surveys2$`OFF													`)
