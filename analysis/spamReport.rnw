...
\end{equation}
where $\Phi^{-1}$ is the inverse acrshort{cdf} of the standard normal distribution and the resulting residuals independently and identically distributed standard normal.

Other parameter setups were tried on the top performing model in an attempt to see if performance could be improved. Setting the correlation parameters of age three cod to zero did not improve the fit to the model. Extending the amount of available catchability parameters, number of standard deviations allowed in $\bm{\sigma}_F}$ and $\bm{\sigma}_{CRL}$ and their corresponding correlation parameters did seem to have a positive impact on the model fit. The selected best model parameter vector mapping are shown in Table \ref{FMPTable}. The better set of parameter mapping reduced the \acrshort{aic} down to 3908.194. The residuals also seem to be marginally improved.
