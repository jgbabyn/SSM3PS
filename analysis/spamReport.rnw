...
Other parameter setups were tried on the top performing model in an attempt to see if performance could be improved. Setting the correlation parameters of age three cod to zero did not improve the fit to the model. Extending the amount of available catchability parameters, number of standard deviations allowed in $\bm{\sigma}_F}$ and $\bm{\sigma}_{CRL}$ and their corresponding correlation parameters did seem to have a positive impact on the model fit. The selected 
...
The final model selected had a ten year retrospective analysis performed to see if any odd patterns emerged as data was removed. 
