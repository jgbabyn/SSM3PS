...
There are too many possible options in \acrshort{spam} to evaluate all of them directly. To narrow down a set of candidate model options, I ran \acrshort{spam} once for every possible option of covariance matrix for the $\bm{F}_y$ random walk and the catch \acrshort{crl}s along with the four possible recruitment options on a single specified mapping of vector parameters shown in table \ref{candTable} with the set of medium landing bounds from Table \ref{landTable}. These choices were somewhat subjective, one common standard deviation parameter was used for the \acrshort{CRL} as more than one seemed to result in extremely large confidence intervals for the time period with no survey data. The \acrfull{aic}, \acrfull{bic}, log-likelihood, time elapsed and convergence status were recorded. 71 of the 100 model runs resulted in relative convergance while the remaining 39 resulted in false or singular convergence and were removed from contention. The top performing models were those that used the custom correlation structure for both the $\bm{F}_y$ random walk and the catch \acrshort{crl}s with different recruitment options. In order to avoid comparing three almost identical models, I will instead focus on the first, fourth and fifth best models which are listed in Table \ref{modChosen}.
...
Model Name & $\bm{F}$ Covariance Matrix Type & CRL Covariance MatrixType & Recruitment Type & AIC      & BIC      \\
Model A    & Custom                          & Custom                    & Random Walk      & 4198.403 & 4426.352 \\
Model B    & AR(1)                           & Custom                    & Ricker           & 4273.728 & 4491.075 \\
Model C    & Custom                          & AR(1)                     & Random Walk      & 4324.045 & 4530.790
\end{tabular}%
}
\caption{Model options of the first, fourth and fith best performing models from the 100 candidates along with their AIC \& BIC.}
\label{modChosen}
\end{table}
...
modelA1 <- spamFit(data,param,indi,crls,random=c("log_N","log_F"),sdrep=FALSE,map=mapA,control=list(iter.max=500,eval.max=500))
rmA1 <- residuals(modelA1)


@

<<modelB,include=FALSE,cache=TRUE>>=
library(SSM3PS)
data(rDat)
...
<<modelC,include=FALSE,cache=TRUE>>=
