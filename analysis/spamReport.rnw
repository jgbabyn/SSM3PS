...
Model designs and the software available to model them have improved greatly since the adoption of SURBA in 2010. \acrfull{ssm} modelling approaches were previously much more difficult to implement and use but advances in software making use of \acrfull{ad} and \acrfull{la} more approachable such as \acrshort{ad} Model Builder \& \acrfull{tmb}\cite{aeberhard2018review}. A \acrshort{ssm} approach has not been formally applied to 3Ps cod and may be able to paint a more accurate picture of the health of the stock.  
\section{Model Design}
\subsection{Process Equations}
The \acrfull{spam} is a state-space stock assessment model implemented in \acrshort{tmb}. As a \acrfull{ssm} the model can be separated into process equations and observation equations. \acrshort{spam} follows the same general process equations design as Nielsen and Berg\cite{Nielsen2014Estimation-of-t}, with the log abundance at age $a$ in a given year $y$, $\log(N_{y,a})$ as 
...
There are too many possible options in \acrshort{spam} to evaluate all of them directly. To narrow down a candidate model option, I ran \acrshort{spam} once for every possible option of covariance matrix for the $\bm{F}_y$ random walk and the catch \acrshort{crl}s along with the four possible recruitment options on a single specified mapping of vector parameters shown in table \ref{candTable} with the set of medium landing bounds from Table \ref{landTable}. These choices were somewhat subjective and somewhat limited in parameter flexibility to reduce computation time, one common standard deviation parameter was used for the \acrshort{crl} as more than one seemed to result in extremely large confidence intervals for the time period with no survey data. The \acrfull{aic}, \acrfull{bic}, log-likelihood, time elapsed and convergence status were recorded. 68 of the 100 model runs resulted in relative convergance while the remaining 32 resulted in false or singular convergence and were removed from contention. The top two performing models were those that used the custom correlation structure for both the $\bm{F}_y$ random walk and the catch \acrshort{crl}s with random walk for recruitment and a Beverton-Holt function. These two models had reduction in \acrshort{aic} of over 80 versus the third best model with an AIC of 4198.403 \& 4205.643 respectively. The worst performing model that converged was parallel correlation structure in both catch \acrshort{CRL}s \& $\bm{F_y}$s with a smooth hockey stick recruitment function. The top performing model had plots and residuals examined to check the fit.

The residuals used in \acrshort{spam} for the observation equations are not standardized Pearson residuals of the form $\frac{y_i-\hat{y_i}}{\sigma_i}$. Standardized Pearson residuals have been shown to be sometimes be misleading and not fully appropriate for \acrshort{ssm}s due to the fact observations are correlated in time\cite{Thygesen2017validation}\cite{aeberhard2018review}. \acrshort{tmb} enables the use of \acrfull{osa} residuals with a some care of how the model is implemented and these are the residuals used in checking \acrshort{spam}. For a series of observations $X_1,...,X_n$, \acrshort{osa} involve the factorization of the joint observations into a series of conditonal distributions given the previous $n-1$ observations like
\begin{equation}
  F_n(x_n) = P(X_n \leq x_n | X_1=x_1,...,X_{n-1} = x_{n-1})
\end{equation}
Then for continous observations the \acrshort{osa} residuals will be
\begin{equation}
  \Phi^{-1}(F_1(X_1)),...,\Phi^{-1}(F_n(X_n))
\end{equation}
where $\Phi^{-1}$ is the inverse acrshort{cdf} of the standard normal distribution and the resulting residuals independently and identically distributed standard normal. The residuals used for evaluating the process equations are joint sample residuals which for some predicted random effects $U$ which is jointly distributed with joint pdf $L(y,u)$ with the observations $Y$ where $y$ is a sample from the marginal distribution of $L(y,u)$. Generating a single sample $u*$ from $U|Y=y$, $u*$ can be used to check the distribution of $U$\cite{Thygesen2017validation}. 

Other parameter setups were tried on the top performing model in an attempt to see if performance could be improved. Setting the correlation parameters of age three cod to zero did not improve the fit to the model. Extending the amount of available catchability parameters, number of standard deviations allowed in $\bm{\sigma}_F}$ and $\bm{\sigma}_{CRL}$ and their corresponding correlation parameters did seem to have a positive impact on the model fit. The selected best model parameter vector mapping are shown in Table \ref{FMPTable}. The better set of parameter mapping reduced the \acrshort{aic} down to 3908.194. The residuals also seem to be marginally improved and are compared to the candidate model, .
...
\resizebox{\textwidth}{!}{%
\begin{tabular}{l}
$\bm{\sigma}_F=(\sigma_{2,F},\sigma_{3,F},\sigma_{4,F},\sigma_{5,F},\sigma_{6,F},\sigma_{7+,F})'$ \\
$\bm{q_{s}}=(q_{s,2},q_{s,3},q_{s,4},q_{s,5},q_{s,6+})'$                                          \\
$\bm{\sigma}_{CRL}=\sigma_{\text{all ages}}$                                                      \\
For Compound and AR(1) for $\bm{F}$, $\rho_F$                                                     \\
For Custom for $\bm{F}$, $\bm{\rho}_F=(\rho_2=0,\rho_3,\rho_4,\rho_5,\rho_6,\rho_{7+})'$          \\
For Compound and AR(1) for CRL, $\rho_{CRL}$                                                      \\
For Custom for CRL, $\bm{\rho}_{CRL}=(\rho_2=0,\rho_3,\rho_4,\rho_5,\rho_6,\rho_{7+})'$         
\end{tabular}%
}


\caption{Setup of parameter vectors used in trying to find the best canadidate options.}
\label{candTable}
\end{table}



<<modelA,include=FALSE>>=
library(SSM3PS)
data(topMod)

qPA1 <- qPattern(topMod$fit$sdr,topMod$fit$indices)

cA1 <- oneStopPlot(topMod$bR,"catch")
RVOA1 <- oneStopPlot(topMod$bR,"RVOff")
RVIA1 <- oneStopPlot(topMod$bR,"RVInOff")
gA1 <- oneStopPlot(topMod$bR,"geac")
fA1 <- oneStopPlot(topMod$bR,"french")
lA1 <- oneStopPlot(topMod$bR,"landings")

cA1r <- oneStopPlot(topMod$resid,"catch",residuals=TRUE)
RVOA1r <- oneStopPlot(topMod$resid,"RVOff",residuals=TRUE)
RVIA1r <- oneStopPlot(topMod$resid,"RVInOff",residuals=TRUE)
gA1r <- oneStopPlot(topMod$resid,"geac",residuals=TRUE)
fA1r <- oneStopPlot(topMod$resid,"french",residuals=TRUE)
lA1r <- oneStopPlot(topMod$resid,"landings",residuals=TRUE)

cA1b <- oneStopPlot(topMod$resid,"catch",bubble=TRUE)
RVOA1b <- oneStopPlot(topMod$resid,"RVOff",bubble=TRUE)
RVIA1b <- oneStopPlot(topMod$resid,"RVInOff",bubble=TRUE)
gA1b <- oneStopPlot(topMod$resid,"geac",bubble=TRUE)
fA1b <- oneStopPlot(topMod$resid,"french",bubble=TRUE)
lA1b <- oneStopPlot(topMod$resid,"landings",bubble=TRUE)

r4pA <- resid4plot(topMod$resid)
r4pA$yplot
r4pA$cplot
r4pA$aplot
r4pA$eplot

ssb <- tsCIplot("log_ssb",topMod$fit$sdr,1959:2016,exp=TRUE)

@

<<modelB,include=FALSE>>=
library(SSM3PS)
data(finMod)

qPA2 <- qPattern(finMod$fit$sdr,finMod$fit$indices)

cA2 <- oneStopPlot(finMod$bR,"catch")
RVOA2 <- oneStopPlot(finMod$bR,"RVOff")
RVIA2 <- oneStopPlot(finMod$bR,"RVInOff")
gA2 <- oneStopPlot(finMod$bR,"geac")
fA2 <- oneStopPlot(finMod$bR,"french")
lA2 <- oneStopPlot(finMod$bR,"landings")

cA2r <- oneStopPlot(finMod$resid,"catch",residuals=TRUE)
RVOA2r <- oneStopPlot(finMod$resid,"RVOff",residuals=TRUE)
RVIA2r <- oneStopPlot(finMod$resid,"RVInOff",residuals=TRUE)
gA2r <- oneStopPlot(finMod$resid,"geac",residuals=TRUE)
fA2r <- oneStopPlot(finMod$resid,"french",residuals=TRUE)
lA2r <- oneStopPlot(finMod$resid,"landings",residuals=TRUE)

cA2b <- oneStopPlot(finMod$resid,"catch",bubble=TRUE)
RVOA2b <- oneStopPlot(finMod$resid,"RVOff",bubble=TRUE)
RVIA2b <- oneStopPlot(finMod$resid,"RVInOff",bubble=TRUE)
gA2b <- oneStopPlot(finMod$resid,"geac",bubble=TRUE)
fA2b <- oneStopPlot(finMod$resid,"french",bubble=TRUE)
lA2b <- oneStopPlot(finMod$resid,"landings",bubble=TRUE)

r4pA2 <- resid4plot(finMod$resid)
r4pA2$yplot
r4pA2$cplot
r4pA2$aplot
r4pA2$eplot
...
\resizebox{\textwidth}{!}{%
\begin{tabular}{l}
$\bm{\sigma}_F=(\sigma_{2,F},\sigma_{3,F},\sigma_{4,F},\sigma_{5,F},\sigma_{6,F},\sigma_{7,F},\sigma_{8,F},\sigma_{9+,F})'$ \\
$\bm{q_{s}}=(q_{s,2},q_{s,3},q_{s,4},q_{s,5},q_{s,6},q_{s,7},q_{s,8},q_{s,9+})'$                                          \\
$\bm{\sigma}_{CRL}=\sigma_{2},\sigma_{3,4,5},\sigma_{6,7,8},\sigma_{9+}$                                                      \\
$\bm{\rho}_F=(\rho_2=0,\rho_3,\rho_4,\rho_5,\rho_6,\rho_{7},\rho_{8})'$          \\
$\bm{\rho}_{CRL}=(\rho_2=0,\rho_3,\rho_4,\rho_5,\rho_6,\rho_{7})'$         
\end{tabular}%
}
\caption{Mapping of parameter vectors in the selected best model.}
\label{FMPTable}
\end{table}
...
The final model selected had a ten year retrospective analysis performed to see if any odd patterns emerged as data was removed. Plots of the ten year retrospective analysis for model predicted \acrshort{ssb},biomass, average fishing mortality ages 4 to 6 and recruitment are shown in Figure \ref{retroFun}. \acrshort{spam} seems to tend to exhibit a pattern of curling upwards as years are removed. Leave one fleet out analysis was also conducted removing one survey fleet at a time and rerunning the model to see if any one of them has a large influence on the results.  

<<retroFun,echo=FALSE,fig.cap="Ten year retropective plots performed SPAM. Confidence intervals are from the final model with all years. SPAM has tendancy to try and recover SSB and Biomass much earlier with less data.">>=
library(SSM3PS)
library(ggplot2)
data(retL)

recP = retL$recRet + ylab("Recruitment (Millions)")
aveP = retL$aveRet + ylab("Average F 4-6")
ssbP = retL$ssbRet + ylab("Spawning Stock Biomass (Kt)")
bioP = retL$bioRet + ylab("Biomass (Kt)")

cowplot::plot_grid(ssbP,bioP,recP,aveP,nrow=2,labels=c("SSB","Bio","Rec.","Ave.F"))
@

<<lfoPlot,echo=FALSE,fig.cap="Plots resulting from leave one fleet out analysis. Removing the DFO RV Inshore and Offshore (Orange Line) has the largest impact on results reversing the upward trend seen in later years.">>=
library(SSM3PS)
library(ggplot2)
data(lfo)

recl = lfo$reclfo + ylab("Recruitment (Millions)")
avel = lfo$avelfo + ylab("Average F 4-6")
ssbl = lfo$ssblfo + ylab("Spawning Stock Biomass (Kt)")
biol = lfo$biolfo + ylab("Biomass (Kt)")

cowplot::plot_grid(ssbl,biol,recl,avel,nrow=2,labels=c("SSB","Bio","Rec.","Ave.F"))
@ 
